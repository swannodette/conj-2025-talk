...
#+begin_src clojure
  (require '[goog.dom :as gdom]
           '[goog.a11y.aria :as aria])
  (import '[goog.a11y.aria State])
  (def button (gdom/getElement "click-me"))
...
#+end_src
* And much, much more
** `goog.a11y.aria` (validation)
** `goog.dom.getActiveElement` (shadow DOM handling)
** `goog.dom.getDocumentHeight` etc.
** `goog.dom.selection` (unified DOM selection API)
** `goog.dom.animation` (split read/write)
** `goog.dom.ViewportSizeMonitor`
** `goog.dom.safe` (sanization)
* Google Closure Library is unfamiliar
** LLMs know a lot about Google Closure Library
** The source is very approachable
*** Boring code
*** Lots of comments
** Project: Hosting GCL docs at the https://clojurescript.org
*** Would be great to have examples for all of the UI components
**** With CLJS translations!
